...
**8.14 Round B (v2) additions (shipped 2026-07-01).** This subsection is a spec addendum, not a rewrite. It records what Round B added on top of the shipped v1. The existing FRs still stand; the items below refine or extend them, and `02_Design_System.md` remains authoritative for all styling values.

*Design System v3 "Calm Premium."* The visual spine moved from a cool Zinc to a warm Stone neutral: canvas `#F7F6F3` (bone), ink `#1C1917`. Shadows became tinted and ultra-diffuse; a muted-pastel `listColors` set replaced the earlier list hues; tabular numerals are used for times and counts; motion is richer and more tactile (a `SPRINGS.tactile` spring and drag physics). A new `FeatureShell` nested-card primitive frames feature sections. The design dials are locked at VARIANCE 5 / MOTION 6 / DENSITY 5. Every hardcoded cool-gray hex was migrated to the warm tokens (zero remain). This supersedes the raw hex mentions in §8.13 for anything not in `02`.

*Calendar drag overhaul (extends FR-28).* Long-press-drag now shows a live snapped-time pill while dragging, fires a selection haptic on each snap step, locks page scroll during a drag with edge autoscroll, and lands with a tactile drop-spring. Pinned blocks show a lock glyph and a Lock/Unlock toggle; resize handles are 44px targets. Cross-day drag in 3-Day and Week is intentionally gated off (it needs a shared cross-column canvas) and is deferred to Round C.

*Todoist-grade task affordances (extends FR-6, §8.5).* The Tasks tab gained swipe actions in both directions, sticky section headers, a long-press `TaskActionSheet`, a list-color tint bar per row, a sticky quick-add, manual drag-reorder (with a screen-reader reorder path), an animated checkbox, and richer row metadata. A unified `Input` primitive backs the editor and quick-add fields.

*Agentic AI planner, on-device (extends FR-84, FR-73).* The project chat is now genuinely agentic client-side. A `ToolAction` discriminated union (`create_task`, `update_task`, `complete_task`, `create_subtasks`, `set_first_move`, `schedule_hint`, `add_project_phase`, `update_project_memory`) is validated and executed on-device by `core/ai-actions.ts`, each with one-tap Undo and a confirmation chip in the chat. `ai-project-chat` returns JSON `{ reply, actions }`. The chat also gained day dividers and a Clear-conversation action. A new `ai-verify-proof` edge function plus a lenient `checkProofPlausibility` (default-pass) backs image verification (FR-77/FR-78). All AI stays behind the no-key contract (`200 { error: "no_key" }` then local fallback); all seven edge functions are deployed and activate the moment `GEMINI_API_KEY` is set. This `ToolAction` vocabulary is deliberately designed to lift verbatim to the future MCP server (FR-73).

*Scheduler depth + a no-shame "missed" view (implements FR-16, FR-20; extends 9.5.12).* Recompute now flips elapsed blocks of still-open tasks to `status: 'missed'`, which feeds a Home "Needs attention" section (per-item **Reschedule** / **Let it go**, no-shame copy) and a Tasks "Missed" filter chip. Scheduling hours resolve per task -> per list -> settings (FR-13). The task editor gained buffer steppers, and the Balanced/Front-load workload toggle (FR-14) is now actually wired to the engine.

*Settings + wellbeing (extends FR-46, FR-63, FR-65).* Per-kind reminder toggles now feed scheduling; a one-time notification-permission nudge replaces nagging; the trial chip animates; Help/About/Legal entries were added. Beat-the-clock (FR-41, Mode C) now defaults OFF and unlocks only after >= 3 successful lock-until-start completions.

**8.14.1 Deferred to Round C+ (explicit, with blockers).** These are deliberate deferrals, not regressions: native iOS Ignition app-locking (blocked on Apple's Family Controls entitlement, weeks); real IAP/StoreKit (needs an Apple developer account); native magic-link deep-link handling (needs a native dev build plus routing/Supabase wiring on a device; web works today); document RAG "ask my syllabus" (pgvector + OCR + embeddings); voice brain-dump STT; home/lock-screen widgets; the MCP server + public API (FR-73/FR-74; the Round B `ToolAction` vocabulary is designed to lift to it verbatim); real-device notification-timing tuning; and calendar-sync OAuth for Google/Outlook/iCloud (FR-22). Cross-day calendar drag in 3-Day/Week is also Round C (see §8.14).
...
*9.5.10 Stability.* Recompute compares new placement to existing blocks and preserves any block whose task and time are unchanged (no churn). Pinned blocks are immovable inputs. Only changed blocks animate. Scheduling-hours resolution per task is `Task.schedulingHours ?? List.schedulingHours ?? Settings.schedulingHours` (FR-13): a task's own window wins, then its list's optional override, then the app default.
...
*9.5.12 Missed marking + "Needs attention" (FR-16, §8.6).* After stability, recompute flips any elapsed block (`end < now`) whose owning task is still open (not `done`/`doing`) to `status: 'missed'`. This is the single source of truth that feeds Recovery detection (`isMissedBlock`/`countMissedBlocks`/`detectLapse`) — the marking pass never adds, removes, or re-times blocks, so it does not affect the pinned-minutes invariant or determinism. Home surfaces missed blocks in a calm "Needs attention" section (shown only when non-empty, before "Scheduled") with two zero-shame inline actions per item: **Reschedule** (drop the block and recompute so the remaining work re-slots forward) and **Let it go** (silently drop the block, task untouched). The Tasks screen exposes a "Missed" filter chip that appears only when something is missed.
...
| 2026-07-01 | Round B: project chat acts via a validated on-device `ToolAction` union (create/update/complete task, subtasks, first move, schedule hint, project phase/memory), each with one-tap Undo; the same vocabulary lifts verbatim to the future MCP server | Aria | Agentic control now, no new server surface, forward-compatible with FR-73 |
| 2026-07-01 | Round B: missed blocks are surfaced in a calm "Needs attention" view with Reschedule / Let it go; recovery stays shame-free | Aria | Turn a missed plan into a one-tap recovery, never a guilt banner |
| 2026-07-01 | Round B: Beat-the-clock (Mode C) defaults OFF and is earned after >= 3 successful lock-until-start completions | Aria | Firm the FR-41 gate to a concrete threshold; avoid meeting the hard mode cold |
| 2026-07-01 | Round B: project file limits = 20 files/project, 25 MB/file, types pdf/img/doc/sheet, enforced in the picker | Aria | Bound storage and ingestion cost; closes the doc-11 GAP |
| 2026-07-01 | Round B: deleting a project keeps and unlinks its generated tasks by default; the confirm dialog offers "also delete tasks" | Aria | Do not silently destroy scheduled work; make deletion reversible in effect |
| 2026-07-01 | Round B: a task belongs to at most one project (one project per task) | Aria | Keep the data model and progress attribution unambiguous |
| 2026-07-01 | Round B: Design System v3 "Calm Premium" (warm Stone spine, bone canvas `#F7F6F3`, ink `#1C1917`, tinted diffuse shadows, muted-pastel lists, tabular numerals, tactile motion); dials locked VARIANCE 5 / MOTION 6 / DENSITY 5 | Aria | Warmer, more premium than the v2 cool-gray pass; a single locked visual target |
