...
- Live auth state (same date): Google sign-in enabled, with the Google Cloud app in **Testing** so only listed test users can complete a sign-in. Apple sign-in is configured in code but not enabled server-side, since it needs a paid Apple Developer account. Email magic link works. Supabase Site URL is still `http://localhost:3000`, which only affects email confirmation links, not the app's `ampora://auth/callback` redirect.
