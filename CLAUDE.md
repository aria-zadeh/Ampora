...
- `supabase/migrations/` holds eight files. **Seven are applied to the live project `pgqbwhksxqgnfdkmwlop` and verified** (checked directly, 2026-08-01): all 11 tables exist, RLS is enabled on every one, and Supabase's own security advisor returns zero lints. The eighth (`20260730000007_drop_energy_columns_PENDING_REVIEW.sql`) is deliberately NOT applied: it drops columns irreversibly and its own header admits the table placement is an unverified guess. `setup/ALL_MIGRATIONS.sql` is a generated single-paste concatenation of the seven runnable ones, for bootstrapping a fresh project.
- Live auth state (re-verified in the dashboard 2026-08-07): Google sign-in enabled, Google Cloud project `ampora-504215`, consent screen still in **Testing** (fine, `aria.nabavi09@gmail.com` is already a listed test user), OAuth client redirect URI correctly `https://pgqbwhksxqgnfdkmwlop.supabase.co/auth/v1/callback`. Apple sign-in is configured in code but not enabled server-side, since it needs a paid Apple Developer account.
- **Both sign-in methods were broken on web until 2026-08-07, from one shared cause.** Supabase Site URL was `http://localhost:3000`, a dead address, and the redirect allowlist held only `ampora://auth/callback`. On web both `signInWithMagicLink` and `signInWithGoogle` send `window.location.origin` (`http://localhost:8081` under Expo), which was not allowlisted, so Supabase rejected it and fell back to that dead Site URL. The earlier note here that Site URL "only affects email confirmation links" was wrong. Fixed by setting Site URL to `ampora://auth/callback` and allowlisting `http://localhost:8081/**`, `http://localhost:8081` (the bare origin, since `/**` does not reliably match a path-less URL) and `ampora://**`. Four redirect URLs total.
- **The sign-in bypass is live on the deployed web build, on purpose.** `FEATURE_FLAGS.DEV_BYPASS_AUTH` is `__DEV__ || EXPO_PUBLIC_DEV_AUTH_BYPASS === '1'`, and `vercel.json` sets that variable, at Aria's explicit request on 2026-08-07, because the deployed URL is not public-facing and sign-in was blocking all use of it. **Remove it from `vercel.json` before the site is public or anything is submitted to a store.** It is tolerable today only because the bypass fabricates no session: no user id, no JWT, sync never runs, every cloud call no-ops, and RLS would reject them anyway, so a visitor gets an empty local-only app and reaches nobody's data.
- Two build facts learned the hard way while wiring that up, both verified rather than assumed:
  - `babel-preset-expo` inlines `EXPO_PUBLIC_*` at transform time, but **Metro's transform cache is not keyed on those values.** A rebuild with a changed variable silently reuses the old inlined value. `vercel.json` passes `--clear` for exactly this reason. Without it the flag inlined to `false` despite being set.
  - Flags read as object properties (`FEATURE_FLAGS.X`) are **not** const-folded, so the guarded JSX still appears in the production bundle even when the flag is false. Grepping a bundle for the string proves nothing about whether it renders. Verify by serving `dist/` and looking, which is what was done here.
