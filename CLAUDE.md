...
- One nuance left open: on native, `signInWithMagicLink` (`services/supabase.ts:144`) leaves `emailRedirectTo` undefined, so the phone case depends on Site URL staying a deep link. Passing `Linking.createURL("auth/callback")` there would make it correct independently of Site URL. Not done yet, deliberately.
